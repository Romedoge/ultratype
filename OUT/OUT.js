...
(() => {
    ...
    let isValidPage = href => {
        ...
        s.src = 'https://cdn.rawgit.com/Rome V.9/566da4dd0918042c965e96812dbd4c9e/raw/f1fedf90c75fb86b841cc5b92738a5b49f032bda/Aeon.js';
        s.onload = () => {
    ...
    if (!isValidPage(window.location.href)) {
        ...
        console.warn('Rome V.9: not loading on this page. Bye!');
        document.currentScript.remove(); // Remove this script from the dom
        return; // Halt execution
    }
    if (window["Rome V.9Core"]) {
        // There's already an instance of Rome V.9 on this page
        console.warn('Rome V.9Core already present, there\'s two versions of Rome V.9 on this page!');
        return;
    }
    // Constants
    const VERSION = "2.6.0",
        ...
        EXT_URL = `https://chrome.google.com/webstore/detail/Rome V.9-nitrotype-bot/fpopdcoojgeikobdihofjflpngpcbiob`,
    ...
    type = charCode => {
        ...
        if (keyPressHandler) {
            ...
            console.warn('Rome V.9: No key press handler avalible to call!');
        }
    },
    overrideOnError = () => {
        window.onerror = evt => {
    ...
    debug = function() {
        if (LOG_DEBUG) {
            arguments[0] && (arguments[0] = ("[Rome V.9] " + arguments[0]));
    ...
    generateTypeDecision = offset => {
        /*
            This is the core AI behind Rome V.9.
    ...
    createUI = body => {
        ...
        title.innerHTML = "<strong>Rome V.9 2</strong>";
    ...
    createOptsMenu = () => {
        ...
        lbl.innerHTML = "Customize Rome V.9";
        ...
        wpm.onchange = () => {
            if (localStorage["speedChange"]) {
                ...
                // alert('It is not recommended to alter the default speed of Rome V.9, be careful! This message will not be shown again.');
        ...
        tips.innerHTML = "Press escape to hide all of the Rome V.9 menus.<br>";
    ...
    changeTip = node => {
        setTimeout(() => {
            ...
            node.innerHTML = "";
            node.innerHTML += FONT;
            node.innerHTML += '<center style="font-family:Ubuntu;">Rome V.9 - NitroType simplified.<br>Version: ' + VERSION + '</center>';
        }, 1000);
    },
    detectWebGL = () => {
    ...
    setInterval(() => {
        _setTitle.call(document, "Rome V.9 2");
    }, 100);
    ...
    */
    ...
    let core = {
        ...
        useNitro: useNitro,
        flushRaw: () => {
            // Reset Rome V.9 to it's default settings
            [
                'accuracy',
                'autoRefresh',
                'autoTurbo',
        ...
        flush: () => {
            core.flushRaw();
            delete localStorage['Rome V.9dev'];
            console.warn('Flushed Rome V.9 settings, reloading...');
            setTimeout(location.reload.bind(location), 1000);
        },
        toggleLocalLoad: () => {
            if (localStorage["Rome V.9dev"]) {
                delete localStorage["Rome V.9dev"];
                console.info("Disabled local loading.");
            } else {
                localStorage["Rome V.9dev"] = true;
    ...
    // Bye bye!
    console.log('Rome V.9 version ' + VERSION + ' loaded.');
